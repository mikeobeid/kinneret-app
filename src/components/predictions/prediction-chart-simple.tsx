...
import { PhytoplanktonPredictor, PredictionData, PredictionResult, PREDICTION_MODELS } from '@/lib/predictions/phytoplankton-predictor';
...
export function PredictionChartSimple({ data, className }: PredictionChartProps) {
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="space-y-2">
              ...
              <select 
                value={selectedGroup} 
                onChange={(e) => setSelectedGroup(e.target.value as keyof Omit<PredictionData, 'date'>)}
            ...
            <div className="space-y-2">
              ...
              <select 
                ...
                onChange={(e) => {
                  const model = PREDICTION_MODELS.find(m => m.name === e.target.value);
              ...
              >
                {PREDICTION_MODELS.map((model) => (
                  <option key={model.name} value={model.name}>
                    {model.name} ({modelAccuracy[model.name]?.toFixed(1) || 0}% accuracy)
                  </option>
                ))}
              </select>
            </div>
